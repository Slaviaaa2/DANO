...
namespace DANO.API
{
    ...
    /// </summary>
    public class DanoGrenade
    {
        // ─── Static ───

        /// <summary>PhysicsGrenade からラッパーを作成する</summary>
        public static DanoGrenade? Get(PhysicsGrenade grenade) =>
            grenade != null ? new DanoGrenade(grenade) : null;

        /// <summary>シーン内の全グレネードを取得</summary>
        public static IEnumerable<DanoGrenade> List =>
            Object.FindObjectsOfType<PhysicsGrenade>().Select(g => new DanoGrenade(g));

        /// <summary>アクティブな（まだ爆発していない）グレネードを取得</summary>
        public static IEnumerable<DanoGrenade> Active =>
            Object.FindObjectsOfType<PhysicsGrenade>()
                .Where(g => g.enabled)
                .Select(g => new DanoGrenade(g));
        ...
        private DanoGrenade(PhysicsGrenade grenade) { Base = grenade; }
        ...
        public override string ToString() => $"DanoGrenade({Name}, Frag={IsFragGrenade}, Stun={IsStunGrenade})";
    }
}
