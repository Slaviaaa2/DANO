...
namespace DANO.API
{
    ...
    public static class GameAPI
    {
        ...
        public static void ScrambleTeams()
        {
            ...
            // 現在のチーム構成を取得して ScrambleTeams に渡す
        ...
        public static void KickPlayer(API.Player player, string message = "")
        {
        ...
        {
            var player = API.Player.Get(playerId);
            if (player != null)
        ...
        {
            return GameManager.Instance?.GetConnectedTeams() ?? System.Array.Empty<int>();
        }
        ...
        public static IEnumerable<API.Player> AlivePlayers =>
            AlivePlayerIds.Select(id => API.Player.Get(id)).Where(p => p != null)!;
