...
namespace DANO.API
{
    ...
    public class Player
    {
        ...
        public static IEnumerable<Player> Alive =>
            List.Where(p => p.IsAlive);

        /// <summary>接続中のプレイヤー数</summary>
        public static int Count => ClientInstance.playerInstances.Count;
        ...
        public static Player? GetBySteamId(ulong steamId) =>
            List.FirstOrDefault(p => p.SteamId == steamId);

        /// <summary>指定プレイヤーが生きているか（IDで検索）</summary>
        public static bool IsAliveById(int playerId) =>
            Get(playerId)?.IsAlive ?? false;

        /// <summary>指定チームのプレイヤー一覧</summary>
        public static IEnumerable<Player> GetTeamMembers(int teamId) =>
            List.Where(p => p.TeamId == teamId);
        ...
        public bool CanMove
        {
            ...
            set { if (Controller != null) Controller.sync___set_value_canMove(value, true); }
        }
        ...
        public void Freeze()
        {
            Controller?.sync___set_value_canMove(false, true);
        }

        /// <summary>移動を再有効化する（アンフリーズ）</summary>
        public void Unfreeze()
        {
            Controller?.sync___set_value_canMove(true, true);
        }
        ...
        {
            var hp = HealthComponent;
            var localHp = Player.Local?.HealthComponent;
        ...
        /// <summary>接続を即時切断する（Kick と異なりメッセージを送らない）</summary>
        ...
        /// <summary>右手または左手に持っているアイテムをドロップさせる</summary>
        public void DropItem(bool rightHand = true)
        {
            ...
            var obj = rightHand ? pickup.objInHand : pickup.objInLeftHand;
            if (obj == null) return;
            pickup.DropObjectServer(obj, rightHand);
        }

        /// <summary>両手に持っているアイテムをすべてドロップさせる</summary>
        public void DropAllItems()
        {
            var pickup = Controller?.GetComponent<PlayerPickup>();
            if (pickup == null) return;
            if (pickup.objInHand != null)
                pickup.DropObjectServer(pickup.objInHand, true);
            if (pickup.objInLeftHand != null)
                pickup.DropObjectServer(pickup.objInLeftHand, false);
        }

        /// <summary>両手に持っているアイテムをすべて取得する</summary>
        public IEnumerable<Item> GetHeldItems()
        {
            var pickup = Controller?.GetComponent<PlayerPickup>();
            if (pickup == null) yield break;
            var right = pickup.objInHand?.GetComponent<ItemBehaviour>();
            if (right != null) yield return Item.Get(right);
            var left = pickup.objInLeftHand?.GetComponent<ItemBehaviour>();
            if (left != null) yield return Item.Get(left);
        }

        /// <summary>武器名でアイテムをスポーンし、プレイヤーの足元に落とす</summary>
        public Item? GiveItem(string weaponName)
        {
            ...
            return prefab != null ? Item.Spawn(prefab, Position + Vector3.up) : null;
        }

        /// <summary>指定プレハブのアイテムをスポーンし、プレイヤーの足元に落とす</summary>
        public Item? GiveItem(GameObject prefab) => Item.Spawn(prefab, Position + Vector3.up);
